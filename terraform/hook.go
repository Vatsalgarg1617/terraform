...
type Hook interface {
	...
	PreApply(addr addrs.ResourceInstance, gen states.Generation, priorState, plannedNewState cty.Value) (HookAction, error)
	PostApply(addr addrs.ResourceInstance, gen states.Generation, newState cty.Value, err error) (HookAction, error)
	...
	// planned new state.
	PreDiff(addr addrs.ResourceInstance, priorState, proposedNewState cty.Value) (HookAction, error)
	PostDiff(addr addrs.ResourceInstance, priorState, plannedNewState cty.Value) (HookAction, error)
	...
	PreProvisionInstance(addr addrs.ResourceInstance, state cty.Value) (HookAction, error)
	PostProvisionInstance(addr addrs.ResourceInstance, state cty.Value) (HookAction, error)
	PreProvisionInstanceStep(addr addrs.ResourceInstance, typeName string) (HookAction, error)
	PostProvisionInstanceStep(addr addrs.ResourceInstance, typeName string, err error) (HookAction, error)
	ProvisionOutput(addr addrs.ResourceInstance, typeName string, line string)

	// PreRefresh and PostRefresh are called before and after a single
	// resource state is refreshed, respectively.
	PreRefresh(addr addrs.ResourceInstance, priorState cty.Value) (HookAction, error)
	PostRefresh(addr addrs.ResourceInstance, priorState cty.Value, newState cty.Value) (HookAction, error)
	...
	PreImportState(addr addrs.ResourceInstance, importID string) (HookAction, error)
	PostImportState(addr addrs.ResourceInstance, imported []*states.ImportedObject) (HookAction, error)
...
func (*NilHook) PreApply(addr addrs.ResourceInstance, gen states.Generation, priorState, plannedNewState cty.Value) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PostApply(addr addrs.ResourceInstance, gen states.Generation, newState cty.Value, err error) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PreDiff(addr addrs.ResourceInstance, priorState, proposedNewState cty.Value) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PostDiff(addr addrs.ResourceInstance, priorState, plannedNewState cty.Value) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PreProvisionInstance(addr addrs.ResourceInstance, state cty.Value) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PostProvisionInstance(addr addrs.ResourceInstance, state cty.Value) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PreProvisionInstanceStep(addr addrs.ResourceInstance, typeName string) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PostProvisionInstanceStep(addr addrs.ResourceInstance, typeName string, err error) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) ProvisionOutput(addr addrs.ResourceInstance, typeName string, line string) {
}

func (*NilHook) PreRefresh(addr addrs.ResourceInstance, priorState cty.Value) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PostRefresh(addr addrs.ResourceInstance, priorState cty.Value, newState cty.Value) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PreImportState(addr addrs.ResourceInstance, importID string) (HookAction, error) {
	return HookActionContinue, nil
}

func (*NilHook) PostImportState(addr addrs.ResourceInstance, imported []*states.ImportedObject) (HookAction, error) {
	return HookActionContinue, nil
}
